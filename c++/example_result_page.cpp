...
set<string> MakeUniqueNonEmptyStrings(const StringContainer& strings) 
{
    ...
    {
        if (!str.empty()) non_empty_strings.insert(str);
    }
...
public:
    ...
    {
        if (!all_of(stop_words_.begin(), stop_words_.end(), IsValidWord)) throw invalid_argument("Some of stop words are invalid"s);
    }
    ...
    {
        if ((document_id < 0) || (documents_.count(document_id) > 0)) throw invalid_argument("Invalid document_id"s);
...
private:
    ...
    vector<string> SplitIntoWordsNoStop(const string& text) const 
    {
        ...
        {
            if (!IsValidWord(word)) throw invalid_argument("Word "s + word + " is invalid"s);
    ...
    QueryWord ParseQueryWord(const string& text) const 
    {
        ...
        if (word.empty() || word[0] == '-' || !IsValidWord(word)) throw invalid_argument("Query word "s + text + " is invalid");
    ...
    Query ParseQuery(const string& text) const 
    {
        ...
        for (const string& word : SplitIntoWords(text)) 
        {
            ...
            {
                if (query_word.is_minus)  result.minus_words.insert(query_word.data);
                else result.plus_words.insert(query_word.data);
            }
        }
        return result;
    }
    ...
    vector<Document> FindAllDocuments(const Query& query, DocumentPredicate document_predicate) const 
    {
        ...
        for (const string& word : query.plus_words) 
        {
            ...
            for (const auto [document_id, term_freq] : word_to_document_freqs_.at(word)) 
            {
                ...
                if (document_predicate(document_id, document_data.status, document_data.rating)) document_to_relevance[document_id] += term_freq * inverse_document_freq;
            }
        }

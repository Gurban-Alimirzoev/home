...
public:
    ...
    explicit Octopus(int num_tentacles) {
            for (int i = 1; i <= num_tentacles; ++i) {
                ...
                tentacles.GetItems().push_back(ptr.Release());
            }            
    }
    ...
    Tentacle& AddTentacle() {        
        auto last = ScopedPtr(new Tentacle(tentacles.GetItems().size() + 1));
        tentacles.GetItems().push_back(last.GetRawPtr());
        return *last.Release();
    }

    int GetTentacleCount() const noexcept {
        return static_cast<int>(tentacles.GetItems().size());
    }

    const Tentacle& GetTentacle(size_t index) const {
        return *tentacles.GetItems().at(index);
    }

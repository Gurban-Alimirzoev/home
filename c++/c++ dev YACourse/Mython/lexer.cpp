...
#undef UNVALUED_OUTPUT
	...
	Lexer::TextLine Lexer::TextLine::ReadLine(istream& in) {
		TextLine line;
	...
	int Lexer::TextLine::SkipSpaces(istream& in) {
		int space_count = 0;
	...
	void Lexer::TextLine::SkipComment(istream& in) {
		for (; !(in.get() == '\n' || in.eof()););
		in.unget();    // вернем последний считанный символ (\n) в поток
	}

	void Lexer::TextLine::ReadString(istream& in, const int begin_quote) {

		auto it = istreambuf_iterator<char>(in);
		...
		tokens_.emplace_back(token_type::String{ move(s) });
	}

	void Lexer::TextLine::ReadNumber(istream& in, const int first_dig) {
		string s{ static_cast<char>(first_dig) };
	...
	void Lexer::TextLine::ReadIdentifier(istream& in, const int first_sym) {
		string s{ static_cast<char>(first_sym) };
		for (int ch = in.get();
		...
		if (s == "class"sv) {
			...
			tokens_.emplace_back(token_type::Id{ move(s) });
		}
	}

	void Lexer::TextLine::ReadComparison(istream& in, const int ch) {
		if (ch == '!') {
	...
	bool Lexer::TextLine::IsEmpty() const {
		return tokens_.empty()
			|| all_of(tokens_.cbegin(), tokens_.cend(), [](const auto& t) {

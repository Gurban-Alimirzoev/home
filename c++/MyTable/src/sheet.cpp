...
void Sheet::SetCell(Position pos, const std::string& text) {
    ...
    ProcessCellSetting(std::move(pos), text);
    InvalidateCache(pos);
}
...
void Sheet::ResizeSheetIfNeeded(Position new_cell_pos) {
    ...
    if (new_cell_pos.col >= static_cast<int>(sheet_[new_cell_pos.row].size())) {
        for (auto& row : sheet_) {
...
void Sheet::ProcessCellSetting(Position pos, std::string text) {
    ...
    if (sheet_[pos.row][pos.col].GetType() == Cell::Type::Formula) {
        ...
        for (const Position& referenced_pos : GetCell(pos)->GetReferencedCells()) {

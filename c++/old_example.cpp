...
public:
    ...
    explicit Cache(ValueFactoryFn value_factory) : Val(value_factory) {}
    ...
    {
        if ((myMap.find(key) == myMap.end()) || myMap[key].expired())
        {
            std::shared_ptr<Value> value = Val(key);
            myMap.insert({key, value});
            return value;
        }
        else 
            return myMap.at(key).lock();
    }

private:
    std::unordered_map<Key, std::weak_ptr<Value>> myMap;
    ValueFactoryFn Val;
    };

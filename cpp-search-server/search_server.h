...
std::vector<Document> SearchServer::FindTopDocuments(std::execution::parallel_policy policy, const std::string_view raw_query, DocumentPredicate document_predicate) const
{
	...
	auto sort_plus = std::async([&query] { std::sort(query.plus_words.begin(), query.plus_words.end()); });
	auto sort_minus = std::async([&query] { std::sort(query.minus_words.begin(), query.minus_words.end()); });
	...
	auto sort_matched_doc = std::async([&matched_documents] { sort(matched_documents.begin(), matched_documents.end(), [](const Document& lhs, const Document& rhs)
		{
			if (std::abs(lhs.relevance - rhs.relevance) < precision) {
				return lhs.rating > rhs.rating;
			}
			...
			if (word_to_document_freqs_.count(word) && !contain_minus) {
				...
				std::for_each(std::execution::par,
					...
					[this, &document_to_relevance, &inverse_document_freq, &document_predicate, &query](const auto& doc_freq)
					{
						...
						if (predicate(doc_freq.first, document_data.status, document_data.rating)) {
